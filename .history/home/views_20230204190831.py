...
class node:
    ...
    def setChildStatus(self, status):
        self.isChild = status
    ...
    for node in node_list:
        ...
        for node2 in node_list[1:]:
            if node2.getParent() == node.getID():
                node2.setChildStatus(True)
                node.addChild(node2)
                #print(node.getChildren())
    
def removeDupeChildren(node_list):
    new_list = []
    for node in node_list:
        if node.getParent == 0:
            new_list.append(node_list.index(node))
